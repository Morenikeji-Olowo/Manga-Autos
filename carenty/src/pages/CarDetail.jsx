...
import { 
  ...
  Zap, Sparkles, Eye, ChevronLeft, ChevronRight
} from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-gray-50 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3">
          <div className="flex items-center gap-1 text-xs sm:text-sm font-body overflow-x-auto whitespace-nowrap">
            ...
            <span className="text-gray-400">/</span>
            <span className="text-gray-900 font-medium truncate">{car.make} {car.model}</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 lg:py-12">
        {/* Mobile: Back button */}
        <Link to="/cars" className="lg:hidden inline-flex items-center gap-1 text-sm text-gray-500 mb-4">
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-2 gap-6 lg:gap-12">
          ...
          <div>
            ...
            <div className="relative bg-gray-100 rounded-xl sm:rounded-2xl overflow-hidden aspect-[4/3]">
              <img
                src={car.images?.[selectedImage] || `https://placehold.co/800x600/e5e5e5/666?text=${car.make}+${car.model}`}
              ...
              <div className="absolute top-3 right-3 flex gap-2">
                <button 
                  ...
                  className="p-2 sm:p-2.5 bg-white/95 backdrop-blur rounded-full hover:bg-white transition shadow-sm"
                >
                  <Heart className={`w-4 h-4 sm:w-5 sm:h-5 ${isWishlisted ? 'fill-red-500 text-red-500' : 'text-gray-700'}`} />
                </button>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mt-4 sm:mt-6">
              <div className="text-center p-2 sm:p-3 bg-gray-50 rounded-lg sm:rounded-xl">
                ...
                <p className="text-sm sm:text-base font-semibold text-gray-900">{details.year || '—'}</p>
              </div>
              <div className="text-center p-2 sm:p-3 bg-gray-50 rounded-lg sm:rounded-xl">
                ...
                <p className="text-sm sm:text-base font-semibold text-gray-900">{details.mileage?.toLocaleString() || '—'} km</p>
              </div>
              <div className="text-center p-2 sm:p-3 bg-gray-50 rounded-lg sm:rounded-xl">
                ...
                <p className="text-sm sm:text-base font-semibold text-gray-900 capitalize">{details.fuelType || '—'}</p>
              </div>
              <div className="text-center p-2 sm:p-3 bg-gray-50 rounded-lg sm:rounded-xl">
                ...
                <p className="text-sm sm:text-base font-semibold text-gray-900 capitalize">{details.transmission || '—'}</p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mb-4 sm:mb-6">
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1 sm:mt-2 font-body">
                    {details.year} • {details.condition === 'excellent' ? '✨ Excellent Condition' : 
                                 details.condition === 'good' ? '✓ Good Condition' : 'Premium Vehicle'}
                  </p>
                </div>
                <div className="text-left sm:text-right">
                  ...
                  {car.negotiable && (
                    <p className="text-xs sm:text-sm text-green-600 font-medium mt-1">Price negotiable</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 text-gray-500 mb-6 pb-4 sm:pb-6 border-b border-gray-100">
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs sm:text-sm font-body">{car.currentLocation || 'Lagos, Nigeria'}</span>
              </div>
              <span className="w-1 h-1 bg-gray-300 rounded-full hidden xs:inline-block"></span>
              <div className="flex items-center gap-1">
                <Clock className="w-3 h-3 sm:w-4 sm:h-4" />
                <span className="text-xs sm:text-sm font-body">Listed {new Date(car.listedDate).toLocaleDateString()}</span>
              </div>
              <span className="w-1 h-1 bg-gray-300 rounded-full hidden xs:inline-block"></span>
              <div className="flex items-center gap-1">
                <Eye className="w-3 h-3 sm:w-4 sm:h-4" />
                <span className="text-xs sm:text-sm font-body">{car.views || 0} views</span>
              </div>
            </div>
            ...
            <div className="mb-6 sm:mb-8 min-h-[280px] sm:min-h-[320px]">
                ...
                <div className="space-y-4 sm:space-y-6">
                  ...
                  {car.benefits?.length > 0 && (
                    <div className="mt-4 p-4 bg-gray-50 rounded-lg sm:rounded-xl">
                      <h4 className="font-semibold text-gray-900 mb-2 sm:mb-3 text-sm sm:text-base">✨ Special Benefits</h4>
                      <div className="flex flex-wrap gap-2">
                        {car.benefits.map((benefit, i) => (
                          <span key={i} className="px-2 sm:px-3 py-1 sm:py-1.5 bg-white rounded-lg text-xs sm:text-sm text-gray-700">
                            {benefit}
                          </span>
                        ))}
                      </div>
              ...
              {activeTab === 'specifications' && (
                <div className="grid grid-cols-1 xs:grid-cols-2 gap-3 sm:gap-4">
                  ...
                  {details.vin && <SpecItem label="VIN" value={details.vin} className="font-mono text-xs" />}
                </div>
              )}
              ...
              {activeTab === 'features' && (
                <div className="space-y-4 sm:space-y-6">
                  ...
                  {car.multimedia?.length > 0 && (
                    <FeatureSection title="Multimedia" icon={Award} items={car.multimedia} />
                  )}
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <div className="mt-4 p-3 sm:p-4 bg-green-50 rounded-lg sm:rounded-xl border border-green-100">
                    <div className="flex items-center gap-2 sm:gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-green-900 text-sm sm:text-base">Vehicle History Verified</p>
                        <p className="text-xs text-green-700 mt-0.5">Reviewed by our team</p>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        {similarCars.length > 0 && (
          <div className="mt-12 sm:mt-16 pt-6 sm:pt-8 border-t border-gray-200">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4 sm:mb-6">
              <div>
                <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Similar Cars</h2>
                <p className="text-sm text-gray-500 mt-1 font-body">You might also like these vehicles</p>
              </div>
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[calc(100%-2rem)] max-w-md z-50 mx-4">
            <div className="bg-white rounded-2xl shadow-xl">
              <div className="p-4 sm:p-5 border-b border-gray-100 flex justify-between items-center">
                <h3 className="text-base sm:text-lg font-semibold text-gray-900">Contact Seller</h3>
              ...
              <div className="p-4 sm:p-5 space-y-3 sm:space-y-4">
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm sm:text-base font-semibold">+234 802 345 6789</p>
                  </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 xs:grid-cols-2 gap-2">
        {items.map((item, i) => (
          <div key={i} className="flex items-center gap-2 text-xs sm:text-sm text-gray-600 font-body">
...
function HistoryItem({ label, value, positiveText, negativeText }) {
  ...
  return (
    <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between gap-1 xs:gap-2 py-2 sm:py-3 border-b border-gray-100">
      <span className="text-xs sm:text-sm text-gray-600 font-body">{label}</span>
      <span className={`text-xs sm:text-sm font-medium ${textColor}`}>{displayText}</span>
    </div>
  )
}
    ...
    <Link to={`/cars/${car._id}`} className="group">
      <div className="bg-white rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 border border-gray-100">
        <div className="aspect-[4/3] overflow-hidden bg-gray-100">
          <img
            src={car.images?.[0] || `https://placehold.co/400x300/e5e5e5/666?text=${car.make}`}
            alt={`${car.make} ${car.model}`}
            className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
          />
        </div>
        <div className="p-3 sm:p-4">
          <h3 className="font-semibold text-gray-900 text-sm sm:text-base">{car.make} {car.model}</h3>
          <p className="text-xs text-gray-500 mt-1">{details.year} • {details.mileage?.toLocaleString()} km</p>
          <p className="text-base sm:text-lg font-bold text-gray-900 mt-2">₦{car.price?.toLocaleString()}</p>
          <p className="text-xs text-gray-500 mt-1 truncate">{car.currentLocation?.split(',')[0]}</p>
        </div>
      </div>
    </Link>
  )
